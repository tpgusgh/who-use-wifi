...
- 포트가 열린 원격 호스트에 한해 `getnameinfo` 역방향 DNS 조회(최대 1초
  타임아웃, 실패하면 그냥 IP만 표시)로 이름을 붙임. 전수 스캔 대상(254개)
  전부가 아니라 열린 포트가 있는 호스트만 조회해서 비용 줄임
- 내 맥 자신은 `ProcessInfo.processInfo.hostName` 그대로 사용

## 자동 새로고침 / 알림

- 상단 토글로 30초 간격 자동 재스캔 on/off
- 매 스캔 결과를 이전 스캔의 "호스트:포트" 집합과 비교해서 새로 나타난
  것만 macOS 알림(`UNUserNotificationCenter`)으로 띄움. 첫 스캔은 비교
  대상이 없으므로 알림 생략

## UI

- 상단: "스캔" 버튼 + 자동 새로고침 토글 + 진행 상태(몇 호스트째 / 전체)
- 본문: IP별로 접었다 펼 수 있는 그룹 리스트, 헤더에 호스트 이름(있으면)
  과 IP를 같이 표시
